...
public class MainActivity extends Activity {
	...
	// Array to store the letters played
	ArrayList<String> lettersPlayed;

	// Variables for storing the word to guess, 
	// the guessed word and the meaning
	String wordToGuess;
	String guessedWord;
	String meaning;
	 ...
	 * @return 0 if the character is valid (is a letter) 1 if the character is
	 *         not a letter 2 if the character is empty 3 if the character is a
	...
	public void inputCharacter(View v) {
		...
		// We store the character in a variable and transform it in the
		// lowercase version
		String strLetter = txtInputLetter.getText().toString()
				.toLowerCase(Locale.getDefault());

		// We validate the character and do different things by the result of
		// the validation
		...
		case 0: {
			...
			lettersPlayed.add(0, strLetter.toUpperCase(Locale.getDefault()));
			
			// Setting the array list to the listview on the activity
			lstLetters.setAdapter(new ArrayAdapter<String>(this,
					android.R.layout.simple_list_item_1, lettersPlayed));

			// Now, we check if the word to guess contains the character input
			// of the player
			if (wordToGuess.indexOf(strLetter) != -1) {

				// We transform the string with the guessed word so far into an
				// array
				...
				for (int i = 0; i < wordToGuess.length(); i++) {
					...
					if (wordToGuess.toCharArray()[i] == strLetter.charAt(0)) {
						// If the find one character that matches, we put it on
						// the guessed word
					...
					// If the player's input is not in this character, we just
					// put the actual
					// value of temp array on it
					guessedWord += temp[i];

					// And do the same with the temp string where we are storing
					// the string
					// showed to the user but with a blank space between
					// characters
					strTemp += temp[i] + " ";
				}

				// Finally, we set the masked string to the textview on the
				// activity
		...
		case 1:
		case 2:
			// Case 1 and 2 for the validate character switch get the same
			// response
		...
		if (state != gameState.CONTINUE) {
			...
			if (state == gameState.WIN) {
				...
				// We show an alert box to the user with a message, the word to
				// guess
				// and it's meaning
	...
	/**
	 * 
	 * Class for retrieving new random words from internet On the onPostExecute
	 * method parser is done to retrieve a word and a meaning from
	 * http://www.randomhouse.com/features/rhwebsters/words.pperl web page
	 * 
	 */
	public class RetrieveNewWord extends AsyncTask<String, Void, String> {
			...
			for (String url : urls) {
				...
				try {
					...
					while ((s = buffer.readLine()) != null) {
						...
						// And adding the lines to the string builder to
						// recreate
		...
		protected void onPostExecute(String result) {
			...
			// We create two pointers, one for the html code before where the
			// word to parse is
			String pointer1 = "<H2>";

			// And the second one with the html code we had after the word we
			// want to parse
			String pointer2 = "</H2>";

			// With that pointers we can search their possitions on the html
			// code of the web page
			int pos1 = html.indexOf(pointer1);
			int pos2 = html.indexOf(pointer2);

			// With that values we now know where is the word we want to parse,
			// and we get it
			...
			if (validateWord(wordToGuess)) {
				...
				// If the word is valid, we use the same procedure to get the
				// word
				...
				// Iterating the length of the word to guess to create the
				// masked strings
